resource "aws_cloudwatch_log_metric_filter" "S3BucketActivity" {
  ...
  log_group_name = aws_cloudwatch_log_group.cloudtrail_all_logs.name
...
resource "aws_cloudwatch_log_metric_filter" "SecurityGroupChangesMetricFilter" {
  ...
  pattern        = "{ ($.eventName = AuthorizeSecurityGroupEgress) || ($.eventName = RevokeSecurityGroupEgress) || ($.eventName = CreateSecurityGroup) || ($.eventName = DeleteSecurityGroup) }"
  log_group_name = aws_cloudwatch_log_group.cloudtrail_all_logs.name
